...
translation = I18n{..}
  where
    ...
    i18nRejectReason (LockFundNotAuthorized LockAccountRejectReasonDetails{..}) = "Account " <> descrAccount account <> " is not authorized to fund lock " <> Text.pack (show lockId) <> "."
    i18nRejectReason (LockSendNotAuthorized LockAccountRejectReasonDetails{..}) = "Account " <> descrAccount account <> " is not authorized to send from lock " <> Text.pack (show lockId) <> "."
    i18nRejectReason (LockReturnNotAuthorized LockAccountRejectReasonDetails{..}) = "Account " <> descrAccount account <> " is not authorized to return from lock " <> Text.pack (show lockId) <> "."
    i18nRejectReason (LockCancelNotAuthorized LockAccountRejectReasonDetails{..}) = "Account " <> descrAccount account <> " is not authorized to cancel lock " <> Text.pack (show lockId) <> "."
    i18nRejectReason (LockTokenNotPermitted LockTokenRejectReasonDetails{..}) = "Token " <> Text.pack (show tokenId) <> " is not permitted by lock " <> Text.pack (show lockId) <> "."
    i18nRejectReason (LockRecipientNotPermitted LockAccountRejectReasonDetails{..}) = "Account " <> descrAccount account <> " is not a permitted recipient for lock " <> Text.pack (show lockId) <> "."
